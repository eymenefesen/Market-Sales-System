...
class urun:
    ...
    def stok_takip(cls):
        ...
        for urun in cls.Urunler:
            if urun.stok != 0:
                print(f"{urun.isim} - {urun.stok} {urun.birim}")
    ...
    def urun_arama(cls,barkod):
        for urun in cls.Urunler:
            if urun.barkod==barkod:
                print(f"{urun.isim} | fiyat - {urun.fiyat} | stok - {urun.stok} {urun.birim}")
                return
    ...
    def urunlistesi(cls):
        ...
        for i in cls.Urunler:
            print(f"ürün - {i.isim} | fiyat - {i.fiyat} | alış fiyatı - {i.alisfiyati} | stok - {i.stok} {i.birim} | barkod - {i.barkod} | kar - {i.fiyat-i.alisfiyati}₺")
            print("-------------------------------------------------------------------------------")             
    ...
    if secenek ==1:
        ...
        for i in urun.Urunler:
            if i.barkod == barkod:
                ...
                if i.stok <= 5 and i.stok != 0:
                    print(f"{i.isim} ürünü'nün stoğu bitmek üzere son {i.stok} {i.birim} kalmıştır")
        ...
        for i in urun.Urunler:
            if i.barkod == barkod:
                eklenen_stok=float(input(f"{i.isim} ürünü'nüne eklemek isteeiğiniz stok sayısını giriniz: "))
